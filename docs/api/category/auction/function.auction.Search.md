---
description: Запрашивает поиск аукционов по заданным параметрам, применяя фильтр, сортировку и пагинацию.
---
...
```lua
auction.Search( filter: table, orderField: number, asc: boolean, page: number )
```
...
Поле сортировки указывается с использованием `AUCTION_ORDERFIELD_...`. Отдельные поля фильтра (например, `itemClass`, `rarityMin`, `rarityMax`) требуют указания строкового псевдонима соответствующего ресурса. Валидность маски для поиска по имени предварительно проверяется функцией `auction.IsSearchNameValid`.
...
- [Константы (AUCTION_ORDERFIELD)](/api/constants/constants.AUCTION_.md#auction-orderfield)
- [События (EVENT_AUCTION_*)](/api/events/events.EVENT_AUCTION_.md#event-auction-search-result)