---
description: Извлекает информацию о состоянии конкретного задания, находящегося в журнале заданий аватара.
---
...
- **`questId`** `(QuestId)` - Уникальный идентификатор квеста, информацию о котором необходимо получить.
