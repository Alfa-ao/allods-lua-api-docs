...
- **`index`** `(number)` - Индекс выполняемого действия из списка доступных действий устройства.

- **`pos`** `(GamePosition)` - Координаты точки местности, к которой применяется выбранное действие.
