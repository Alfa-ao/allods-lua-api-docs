...
- **`itemId`** (`ObjectId`) - Идентификатор предмета, который необходимо использовать.

- **`count`** (`number`) - Количество предметов из стека, подвергаемых использованию. По умолчанию принимает значение `1`.

- **`useAction`** (`number`) - Индекс (`sysIndex`) выбранного действия использования (`Usage`). По умолчанию принимает значение `-1`.
