...
- **`sysSectionName`** (`string`) - Идентификатор секции, к которой принадлежит бинд.

- **`sysBindName`** (`string`) - Идентификатор (служебное название) искомого бинда.
...
- **`keys`** (`table`) - Список комбинаций клавиш бинда. Индексация начинается с `0`. Может содержать произвольное количество элементов (вплоть до нуля).
