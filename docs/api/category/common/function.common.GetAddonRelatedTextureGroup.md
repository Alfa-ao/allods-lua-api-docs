---
description: Возвращает группу связанных с аддоном текстурных ресурсов.
---
...
- **`sysGroup`** (`string`) - Идентификатор (имя) искомой группы текстурных ресурсов.

- **`optional`** (`boolean` | `nil`) - Флаг, определяющий поведение при отсутствии искомой группы. Если передано `true`, отсутствие группы не считается ошибкой и не вызывает предупреждений в логах (по умолчанию `false`).
