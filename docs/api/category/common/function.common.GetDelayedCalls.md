---
description: Получает информацию о запланированных вызовах для текущего аддона.
---
...
- **`executeTimestamp`** `(number` | `int64)` - Отметка времени, когда произойдет вызов (не ранее указанного времени, возможна задержка до 2 кадров).

- **`function`** `(function)` - Запланированная к вызову функция.

- **`functionRef`** `(number` | `int)` - Ссылка на функцию, по которой можно отменить вызов.

- **`arguments`** `(table)` - Список аргументов запланированного вызова (от 0 до 10 штук, индексация с 1).
