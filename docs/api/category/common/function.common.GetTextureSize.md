...
- **`textureId`** `(TextureId)` - Идентификатор ресурса текстуры, размеры которой необходимо получить.

## Возвращаемые значения

Возвращает `table` или `nil`:
- **`sizeX`** `(number)` - Реальная ширина текстуры.

- **`sizeY`** `(number)` - Реальная высота текстуры.
