...
```lua
WidgetSafe:PlayRotationWithChildrenEffect( start: table|nil, finish: table|nil, timeMs: number|nil, algoDesc: number|table|nil, needEvent: boolean|nil, eventReceivers: table|nil )
```
...
Работает аналогично `PlayRotationEffect`, но применяется не только к целевому виджету, но и рекурсивно к его детям.
...
```lua
wt:PlayRotationWithChildrenEffect( 0, math.pi, 2000 )
```
