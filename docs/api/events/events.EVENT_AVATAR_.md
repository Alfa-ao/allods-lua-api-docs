...
Событие присылается в случае невозможности снять бафф с аватара с помощью `object.RemoveBuff( buffIndex )`.
...
Событие срабатывает, когда текущая цель аватара меняет свой таргет.

::: tip
Это действительно такое событие существует.
:::
