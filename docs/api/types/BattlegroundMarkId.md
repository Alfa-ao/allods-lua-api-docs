---
description: Идентификатор команды, принимающей участие в баттлграунде. Один из типов группы `ResourceId`.
---

# BattlegroundMarkId

Идентификатор команды, принимающей участие в баттлграунде. Один из типов группы `ResourceId`.

Поля, доступные в [BattlegroundMarkId:GetInfo](/api/category/ResourceId/method.GetInfo.md)

```
["image"] => userdata(UITextureId) = {} - идентификатор иконки
["sysName"] => string(4) "Blue" - системное название команды
["sysTeam"] => string(20) "ENUM_FightTeam_FIRST"
["team"] => number(0)
```